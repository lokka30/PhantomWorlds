...
public class TeleportSubcommand implements Subcommand {
  ...
  public void parseCommand(CommandSender sender, Command cmd, String label, String[] args) {
    ...
    if(!sender.hasPermission("phantomworlds.command.phantomworlds." + subCommandLabel)) {
      (new MultiMessage(
              ...
              new MultiMessage.Placeholder("permission",
                      "phantomworlds.command.phantomworlds." + subCommandLabel, false)
      ))).send(sender);
      return;
    }
    ...
    switch(subCommandLabel) {
      case "teleport":
        if(!(args.length == 2 || args.length == 3)) {
          (new MultiMessage(
                  PhantomWorlds.instance().messages.getConfig().getStringList(
                          "command.phantomworlds.subcommands." + subCommandLabel + ".usage"),
                  Arrays.asList(
        ...
        teleportToWorld(sender, subCommandLabel, label, targetPlayerName, args[1]);
      ...
      case "spawn":
        ...
        teleportToWorld(sender, subCommandLabel, label, targetPlayerName, null);
  ...
  private void teleportToWorld(@NotNull CommandSender sender, @NotNull String subCommand,
